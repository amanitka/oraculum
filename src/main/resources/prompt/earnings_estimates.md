...
- CRITICAL EVIDENCE UNIT & COMPARISON RULE: `value` and `previous_value` in the `evidence` array MUST share the exact same metric unit, scale, and meaning! If `value` is a percentage change or EPS estimate (e.g. '$2.45' or '15%'), `previous_value` MUST be the previous estimate or actual (e.g. '$2.10' or '12%'), NOT an unrelated index figure. Never compare percentages/estimates with raw index or dollar values of different metrics.
