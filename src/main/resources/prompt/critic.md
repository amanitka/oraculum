...
You will be provided with a JSON object containing the outputs of several specialist agents (e.g., `Valuation`,
`Fundamentals`, `SharePrice`).
...
4. **Set Consistency Flag**: If `contradictions_found` is empty, set `is_consistent` to `true`. Otherwise, set it to
   `false`.
