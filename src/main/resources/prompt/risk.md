...
3.  `daily_share_price_signals`: JSON data on recent and historical price action.

Your task is to:
1.  **Analyze Leverage and Liquidity**: Examine the `balance_sheet_history` and `company_financial_ratios`. Assess debt levels (`total_debt`), debt-to-equity, and current ratio.
2.  **Identify Key Risks**: Scrutinize all provided data for potential risks. Examples include:
    ...
    *   A share price in a steep, prolonged downtrend (from `daily_share_price_signals`).
