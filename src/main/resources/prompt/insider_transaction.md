...
- CRITICAL EVIDENCE UNIT & COMPARISON RULE: `value` and `previous_value` in the `evidence` array MUST share the exact same metric unit, scale, and meaning! If `value` is a transaction volume or ratio (e.g. '$1.77B' or '5 buys'), `previous_value` MUST be the previous period's volume or ratio (e.g. '$0.50B' or '2 buys'), NOT an unrelated price figure. Never compare percentages with raw counts or dollar levels.
