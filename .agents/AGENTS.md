...
Welcome to the `oraculum` repository! As an AI coding assistant, you **MUST** adhere to the following architectural and
stylistic rules when modifying this codebase.
...
- Do **not** create tangled cross-module dependencies. Inter-module communication should happen via Spring Application
  Events (`@Async` event listeners) or clean, well-defined API interfaces.

## 2. DRY & KISS Principles

- **DRY (Don't Repeat Yourself)**: Extract common logic into shared utility methods, default interface methods, or
  abstract base classes. Do not copy-paste code.
- **KISS (Keep It Simple, Stupid)**: Avoid over-engineering. Write straightforward, readable code over "clever" but
  complex code.

## 3. SOLID Principles

- Ensure your code follows SOLID principles (Single Responsibility, Open/Closed, Liskov Substitution, Interface
